...
function criarElementoResultadoErro(value){
    const result =document.querySelector('#result')
    result.innerHTML=""
    result.insertAdjacentHTML("beforeend", `<h2 style="color: #F00">${value}</h2>`
    );
}
...
function pesquisaCep(cep) {
    ...
    .then((result) => {criarElementoResultadoSucesso(result)})
    .catch((err) => {criarElementoResultadoErro("CEP inválido!!")})
}
